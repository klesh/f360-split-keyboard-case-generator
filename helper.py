...
def add_box(sketch: adsk.fusion.Sketch, box: Box):
    sketch.sketchCurves.sketchLines.addTwoPointRectangle(
        add_point(sketch, box.p1), add_point(sketch, box.p2),
    )


def add_circle(sketch: adsk.fusion.Sketch, center: Point, radius: float):
    sketch.sketchCurves.sketchCircles.addByCenterRadius(add_point(sketch, center), radius)


def connect_points(sketch, points, close=False):
    l = len(points)
    lines = map(lambda i: (points[i], points[(i + 1) % l]), range(l if close else l - 1))
    return [sketch.sketchCurves.sketchLines.addByTwoPoints(p1, p2) for p1, p2 in lines]
...
def add_tangent_circle(sketch, point: adsk.fusion.SketchPoint, radius: float):
    ...
    center = adsk.core.Point3D.create(
        (p1.geometry.x + p2.geometry.x) / 2, (p1.geometry.y + p2.geometry.y) / 2, (p1.geometry.z + p2.geometry.z) / 2
    )
...
def get_offsetters(profiles):
    return [
        (to_collection(map(lambda pc: pc.sketchEntity, p.profileLoops[0].profileCurves)), p.areaProperties().centroid,)
        for p in profiles
    ]
